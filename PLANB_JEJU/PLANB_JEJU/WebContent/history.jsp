<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
    
<link rel="shortcut icon" href="favicon.ico">

<link href='https://fonts.googleapis.com/css?family=Open+Sans:400,700,300' rel='stylesheet' type='text/css'>
	
<!-- Icomoon Icon Fonts-->
<link rel="stylesheet" href="css/icomoon.css">
<!-- Bootstrap  -->
<link rel="stylesheet" href="css/bootstrap.css">
<!-- Superfish -->
<link rel="stylesheet" href="css/superfish.css">
<!-- histroy css -->
<link rel="stylesheet" href="css/history.css">

	<!-- <header id="fh5co-header-section" class="sticky-banner">
		<div class="container">
			<div class="nav-header">
			<h1 id="fh5co-logo"><a href="index.html"><i class="icon-airplane"></i>PLAN'B 제주</a></h1>
				<a href="#" class="js-fh5co-nav-toggle fh5co-nav-toggle dark"><i></i></a>
			<nav id="fh5co-menu-wrap" role="navigation">
				<ul class="sf-menu" id="fh5co-primary-menu">
					<li class="active"><a href="index.html">일정 만들기</a></li>
					<li><a href="flight.html">후기 게시판</a></li>
					<li><a href="hotel.html">마이 페이지</a></li>
					<li><a href="car.html">로그아웃</a></li>
				</ul>
			</nav>
	  	 	</div>
	  	 	<hr>
		</div>
	 </header> -->


<div class="page_left" >
	<div class="day_box" data-id="1" >
		 
		<div class="day_info_box">
			<div class="day_txt">DAY1</div>
			<div class="day_info">
				<div class="day_info_left">
					<!-- 
					<div class="date">2017.06.12 (월)</div>
					<div class="day_title">제주도</div> 
					-->
					<div class="date"></div>
					<div class="day_title">2017.06.12 (월)</div>
				</div>
				<div class="day_info_right" style="display: none;"></div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
		</div> 
		<div class="clear"></div>
		<div class="day_sch_box" data-id="1-1">
			<div class="day_sch_num">
				<div class="sch_num">1</div>
			</div>
			<div class="sch_content">
				<img src="http://img.earthtory.com/img/place_img/312/7505_0_et.jpg"
					alt="" class="spot_img"
					onerror="this.src='/res/img/common/no_img/sight55.png';"
					onclick="window.open('/ko/city/jeju_312/attraction/yongdam-ocean-road_7505');"
					style="cursor: pointer;">
				<div class="spot_content_box">
					<div class="spot_name"
						onclick="window.open('/ko/city/jeju_312/attraction/yongdam-ocean-road_7505');"
						style="cursor: pointer;">용담 해안도로</div>
					<div class="spot_info">
						<div class="tag">유명한거리/지역</div>
						<div class="sinfo_line"></div>
						<div class="sinfo_txt">
						<img src="css/history/like.png" style="height : 20px">
							6 / 10 <span>1개의 평가</span>
						</div>
						<div class="clear"></div>
					</div>
				</div>
				<div class="spot_btn_box">
					<img src="css/history/map_ico.png" alt="" class="spot_btn map_view" onclick="set_center(33.51010100,126.48125500)">
					<img src="css/history/info_ico.png" alt=""	class="spot_btn spot_info_btn"
						onclick="window.open('/ko/city/jeju_312/attraction/yongdam-ocean-road_7505');">
					<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
			<div class="sch_add_content">
				<div class="sch_price hide" data-srl="1-1"></div>
				<div class="sch_memo"
					data-full="&nbsp;<span class='memo_more' data-op='min'>닫기</span>"
					data-min="" data-srl="1-1"></div>	
			</div>
			<div class="clear"></div>
		</div>
		<div>
		
		</div>
		<div class="day_sch_distance" >
		<img src="css/history/down.png" style="height : 20px">	
			
			<a>&emsp;&emsp;&emsp;&emsp;  11.16km 추천경로</a>
		</div>
		<div class="day_sch_box" data-id="1-2">
			<div class="day_sch_num">
				<div class="sch_num">2</div>
			</div>
			<div class="sch_content">
				<img src="http://img.earthtory.com/img/place_img/312/7504_0_et.jpg"
					alt="" class="spot_img"
					onerror="this.src='/res/img/common/no_img/sight55.png';"
					onclick="window.open('/ko/city/jeju_312/attraction/aewol-ocean-road_7504');"
					style="cursor: pointer;">
				<div class="spot_content_box">
					<div class="spot_name"
						onclick="window.open('/ko/city/jeju_312/attraction/aewol-ocean-road_7504');"
						style="cursor: pointer;">애월 해안도로</div>
					<div class="spot_info">
						<div class="tag">유명한거리/지역</div>
						<div class="sinfo_line"></div>
						<div class="sinfo_txt">
						<img src="css/history/like.png" style="height : 20px">
							8 / 10 <span>4개의 평가</span>
						</div>
						<div class="clear"></div>
					</div>
				</div>
				<div class="spot_btn_box">
					<img src="css/history/map_ico.png" alt=""
						class="spot_btn map_view"
						onclick="set_center(33.47822600,126.36708100)"><img
						src="css/history/info_ico.png" alt=""
						class="spot_btn spot_info_btn"
						onclick="window.open('/ko/city/jeju_312/attraction/aewol-ocean-road_7504');">
					<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
			<div class="sch_add_content">
				<div class="sch_price hide" data-srl="1-2"></div>
				<div class="sch_memo"
					data-full="&nbsp;<span class='memo_more' data-op='min'>닫기</span>"
					data-min="" data-srl="1-2"></div>
				
			</div>
			<div class="clear"></div>
		</div>
		<div class="day_sch_distance">
		<img src="css/history/down.png" style="height : 20px">
			
			<a
				href="javascript:et_modal('620px','700px','1','0','/ko/modal/directions?from_latlng=33.47822600,126.36708100&amp;from_name=애월 해안도로&amp;to_latlng=33.39410800,126.23978400&amp;to_name=협재 해수욕장','2','1');">&emsp;&emsp; &emsp;&emsp; 
				15.06km 추천경로</a>
		</div>
		<div class="day_sch_box" data-id="1-3">
			<div class="day_sch_num">
				<div class="sch_num">3</div>
			</div>
			<div class="sch_content">
				<img src="http://img.earthtory.com/img/place_img/312/7281_2_et.jpg"
					alt="" class="spot_img"
					onerror="this.src='/res/img/common/no_img/sight55.png';"
					onclick="window.open('/ko/city/jeju_312/attraction/hyeopjae-beach_7281');"
					style="cursor: pointer;">
				<div class="spot_content_box">
					<div class="spot_name"
						onclick="window.open('/ko/city/jeju_312/attraction/hyeopjae-beach_7281');"
						style="cursor: pointer;">협재 해수욕장</div>
					<div class="spot_info">
						<div class="tag">해변/항구</div>
						<div class="sinfo_line"></div>
						<div class="sinfo_txt">
						<img src="css/history/like.png" style="height : 20px">
							9.2 / 10 <span>5개의 평가</span>
						</div>
						<div class="clear"></div>
					</div>
				</div>
				<div class="spot_btn_box">
					<img src="css/history/map_ico.png" alt=""
						class="spot_btn map_view"
						onclick="set_center(33.39410800,126.23978400)"><img
						src="css/history/info_ico.png" alt=""
						class="spot_btn spot_info_btn"
						onclick="window.open('/ko/city/jeju_312/attraction/hyeopjae-beach_7281');">
					<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
			<div class="sch_add_content">
				<div class="sch_price hide" data-srl="1-3"></div>
				<div class="sch_memo"
					data-full="&nbsp;<span class='memo_more' data-op='min'>닫기</span>"
					data-min="" data-srl="1-3"></div>
			
			</div>
			<div class="clear"></div>
		</div>
		<div class="day_sch_distance">
		<img src="css/history/down.png" style="height : 20px">	
			<a
				href="javascript:et_modal('620px','700px','1','0','/ko/modal/directions?from_latlng=33.39410800,126.23978400&amp;from_name=협재 해수욕장&amp;to_latlng=33.34296839,126.17262312&amp;to_name=신창풍차해안도로','2','1');">&emsp;&emsp; &emsp;&emsp; 
				8.43km 추천경로</a>
		</div>
		<div class="day_sch_box" data-id="1-4">
			<div class="day_sch_num">
				<div class="sch_num">4</div>
			</div>
			<div class="sch_content">
				<img src="http://img.earthtory.com/img/place_img/312/13066_0_et.jpg"
					alt="" class="spot_img"
					onerror="this.src='/res/img/common/no_img/sight55.png';"
					onclick="window.open('/ko/city/jeju_312/attraction/shinchang-windmill-coast-road_13066');"
					style="cursor: pointer;">
				<div class="spot_content_box">
					<div class="spot_name"
						onclick="window.open('/ko/city/jeju_312/attraction/shinchang-windmill-coast-road_13066');"
						style="cursor: pointer;">신창풍차해안도로</div>
					<div class="spot_info">
						<div class="tag">자연절경</div>
						<div class="sinfo_line"></div>
						<div class="sinfo_txt">
						<img src="css/history/like.png" style="height : 20px">
							6 / 10 <span>1개의 평가</span>
						</div>
						<div class="clear"></div>
					</div>
				</div>
				<div class="spot_btn_box">
					<img src="css/history/map_ico.png" alt=""
						class="spot_btn map_view"
						onclick="set_center(33.34296839,126.17262312)"><img
						src="css/history/info_ico.png" alt=""
						class="spot_btn spot_info_btn"
						onclick="window.open('/ko/city/jeju_312/attraction/shinchang-windmill-coast-road_13066');">
					<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
			<div class="sch_add_content">
				<div class="sch_price hide" data-srl="1-4"></div>
				<div class="sch_memo"
					data-full="&nbsp;<span class='memo_more' data-op='min'>닫기</span>"
					data-min="" data-srl="1-4"></div>
			
			</div>
			<div class="clear"></div>
		</div>
		<div class="day_sch_distance">
		<img src="css/history/down.png" style="height : 20px">	
			<a
				href="javascript:et_modal('620px','700px','1','0','/ko/modal/directions?from_latlng=33.34296839,126.17262312&amp;from_name=신창풍차해안도로&amp;to_latlng=33.30584000,126.28942900&amp;to_name=오설록 티 뮤지엄','2','1');">&emsp;&emsp; &emsp;&emsp; 
				11.60km 추천경로</a>
		</div>
		<div class="day_sch_box" data-id="1-5">
			<div class="day_sch_num">
				<div class="sch_num">5</div>
			</div>
			<div class="sch_content">
				<img src="http://img.earthtory.com/img/place_img/312/7326_0_et.jpg"
					alt="" class="spot_img"
					onerror="this.src='/res/img/common/no_img/sight55.png';"
					onclick="window.open('/ko/city/jeju_312/attraction/osulloc-tea-museum_7326');"
					style="cursor: pointer;">
				<div class="spot_content_box">
					<div class="spot_name"
						onclick="window.open('/ko/city/jeju_312/attraction/osulloc-tea-museum_7326');"
						style="cursor: pointer;">오설록 티 뮤지엄</div>
					<div class="spot_info">
						<div class="tag">박물관</div>
						<div class="sinfo_line"></div>
						<div class="sinfo_txt">
						<img src="css/history/like.png" style="height : 20px">
							6.2 / 10 <span>7개의 평가</span>
						</div>
						<div class="clear"></div>
					</div>
				</div>
				<div class="spot_btn_box">
					<img src="css/history/map_ico.png" alt=""
						class="spot_btn map_view"
						onclick="set_center(33.30584000,126.28942900)"><img
						src="css/history/info_ico.png" alt=""
						class="spot_btn spot_info_btn"
						onclick="window.open('/ko/city/jeju_312/attraction/osulloc-tea-museum_7326');">
					<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
			<div class="sch_add_content">
				<div class="sch_price hide" data-srl="1-5"></div>
				<div class="sch_memo"
					data-full="&nbsp;<span class='memo_more' data-op='min'>닫기</span>"
					data-min="" data-srl="1-5"></div>
			
			</div>
			<div class="clear"></div>
		</div>
	</div>
	<div class="day_box" data-id="2">
		<div class="day_info_box">
			<div class="day_txt">DAY2</div>
			<div class="day_info">
				<div class="day_info_left">
					<div class="date">2017.06.13 (화)</div>
					<div class="day_title">제주도</div>
				</div>
				<div class="day_info_right" style="display: none;"></div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
		</div>
		<div class="clear"></div>
		<div class="day_sch_box" data-id="2-1">
			<div class="day_sch_num">
				<div class="sch_num">1</div>
			</div>
			<div class="sch_content">
				<img src="http://img.earthtory.com/img/place_img/312/7401_0_et.jpg"
					alt="" class="spot_img"
					onerror="this.src='/res/img/common/no_img/sight55.png';"
					onclick="window.open('/ko/city/jeju_312/attraction/moseulpo-port_7401');"
					style="cursor: pointer;">
				<div class="spot_content_box">
					<div class="spot_name"
						onclick="window.open('/ko/city/jeju_312/attraction/moseulpo-port_7401');"
						style="cursor: pointer;">모슬포항</div>
					<div class="spot_info">
					
						<div class="tag">해변/항구</div>
						
						<div class="clear"></div>
					</div>
				</div>
				<div class="spot_btn_box">
					<img src="css/history/map_ico.png" alt=""
						class="spot_btn map_view"
						onclick="set_center(33.21435900,126.25169400)"><img
						src="css/history/info_ico.png" alt=""
						class="spot_btn spot_info_btn"
						onclick="window.open('/ko/city/jeju_312/attraction/moseulpo-port_7401');">
					<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
			<div class="sch_add_content">
				<div class="sch_price hide" data-srl="2-1"></div>
				<div class="sch_memo"
					data-full="&nbsp;<span class='memo_more' data-op='min'>닫기</span>"
					data-min="" data-srl="2-1"></div>
			
			</div>
			<div class="clear"></div>
		</div>
		<div class="day_sch_distance">
		<img src="css/history/down.png" style="height : 20px">	
			<a
				href="javascript:et_modal('620px','700px','1','0','/ko/modal/directions?from_latlng=33.21435900,126.25169400&amp;from_name=모슬포항&amp;to_latlng=33.22432300,126.29802700&amp;to_name=사계 해안도로','2','1');">&emsp;&emsp; &emsp;&emsp; 
				4.45km 추천경로</a>
		</div>
		<div class="day_sch_box" data-id="2-2">
			<div class="day_sch_num">
				<div class="sch_num">2</div>
			</div>
			<div class="sch_content">
				<img src="http://img.earthtory.com/img/place_img/312/7506_0_et.jpg"
					alt="" class="spot_img"
					onerror="this.src='/res/img/common/no_img/sight55.png';"
					onclick="window.open('/ko/city/jeju_312/attraction/sagye-ocean-road_7506');"
					style="cursor: pointer;">
				<div class="spot_content_box">
					<div class="spot_name"
						onclick="window.open('/ko/city/jeju_312/attraction/sagye-ocean-road_7506');"
						style="cursor: pointer;">사계 해안도로</div>
					<div class="spot_info">
						<div class="tag">유명한거리/지역</div>
						<div class="clear"></div>
					</div>
				</div>
				<div class="spot_btn_box">
					<img src="css/history/map_ico.png" alt=""
						class="spot_btn map_view"
						onclick="set_center(33.22432300,126.29802700)"><img
						src="css/history/info_ico.png" alt=""
						class="spot_btn spot_info_btn"
						onclick="window.open('/ko/city/jeju_312/attraction/sagye-ocean-road_7506');">
					<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
			<div class="sch_add_content">
				<div class="sch_price hide" data-srl="2-2"></div>
				<div class="sch_memo"
					data-full="&nbsp;<span class='memo_more' data-op='min'>닫기</span>"
					data-min="" data-srl="2-2"></div>
				
			</div>
			<div class="clear"></div>
		</div>
		<div class="day_sch_distance">
		<img src="css/history/down.png" style="height : 20px">	
			<a
				href="javascript:et_modal('620px','700px','1','0','/ko/modal/directions?from_latlng=33.22432300,126.29802700&amp;from_name=사계 해안도로&amp;to_latlng=33.23747600,126.42509500&amp;to_name=중문·대포해안 주상 절리대','2','1');">&emsp;&emsp; &emsp;&emsp; 
				11.90km 추천경로</a>
		</div>
		<div class="day_sch_box" data-id="2-3">
			<div class="day_sch_num">
				<div class="sch_num">3</div>
			</div>
			<div class="sch_content">
				<img src="http://img.earthtory.com/img/place_img/312/7321_0_et.jpg"
					alt="" class="spot_img"
					onerror="this.src='/res/img/common/no_img/sight55.png';"
					onclick="window.open('/ko/city/jeju_312/attraction/jusangjeolli-cliff-of-daepojungmun-daepo-beach-jusang-jeollidae_7321');"
					style="cursor: pointer;">
				<div class="spot_content_box">
					<div class="spot_name"
						onclick="window.open('/ko/city/jeju_312/attraction/jusangjeolli-cliff-of-daepojungmun-daepo-beach-jusang-jeollidae_7321');"
						style="cursor: pointer;">중문·대포해안 주상 절리대</div>
					<div class="spot_info">
						<div class="tag">세계문화유산</div>
						<div class="sinfo_line"></div>
						<div class="sinfo_txt">
						<img src="css/history/like.png" style="height : 20px">
							10 / 10 <span>1개의 평가</span>
						</div>
						<div class="clear"></div>
					</div>
				</div>
				<div class="spot_btn_box">
					<img src="css/history/map_ico.png" alt=""
						class="spot_btn map_view"
						onclick="set_center(33.23747600,126.42509500)"><img
						src="css/history/info_ico.png" alt=""
						class="spot_btn spot_info_btn"
						onclick="window.open('/ko/city/jeju_312/attraction/jusangjeolli-cliff-of-daepojungmun-daepo-beach-jusang-jeollidae_7321');">
					<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
			<div class="sch_add_content">
				<div class="sch_price hide" data-srl="2-3"></div>
				<div class="sch_memo"
					data-full="&nbsp;<span class='memo_more' data-op='min'>닫기</span>"
					data-min="" data-srl="2-3"></div>
			</div>
			<div class="clear"></div>
		</div>
		<div class="day_sch_distance">
		<img src="css/history/down.png" style="height : 20px">	
			<a
				href="javascript:et_modal('620px','700px','1','0','/ko/modal/directions?from_latlng=33.23747600,126.42509500&amp;from_name=중문·대포해안 주상 절리대&amp;to_latlng=33.24727600,126.55482500&amp;to_name=천지연 폭포','2','1');">&emsp;&emsp; &emsp;&emsp; 
				12.11km 추천경로</a>
		</div>
		<div class="day_sch_box" data-id="2-4">
			<div class="day_sch_num">
				<div class="sch_num">4</div>
			</div>
			<div class="sch_content">
				<img src="http://img.earthtory.com/img/place_img/312/7319_0_et.jpg"
					alt="" class="spot_img"
					onerror="this.src='/res/img/common/no_img/sight55.png';"
					onclick="window.open('/ko/city/jeju_312/attraction/cheonjiyeon-falls_7319');"
					style="cursor: pointer;">
				<div class="spot_content_box">
					<div class="spot_name"
						onclick="window.open('/ko/city/jeju_312/attraction/cheonjiyeon-falls_7319');"
						style="cursor: pointer;">천지연 폭포</div>
					<div class="spot_info">
						<div class="tag">자연절경</div>
						<div class="sinfo_line"></div>
						<div class="sinfo_txt">
						<img src="css/history/like.png" style="height : 20px">
							10 / 10 <span>1개의 평가</span>
						</div>
						<div class="clear"></div>
					</div>
				</div>
				<div class="spot_btn_box">
					<img src="css/history/map_ico.png" alt=""
						class="spot_btn map_view"
						onclick="set_center(33.24727600,126.55482500)"><img
						src="css/history/info_ico.png" alt=""
						class="spot_btn spot_info_btn"
						onclick="window.open('/ko/city/jeju_312/attraction/cheonjiyeon-falls_7319');">
					<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
			<div class="sch_add_content">
				<div class="sch_price hide" data-srl="2-4"></div>
				<div class="sch_memo"
					data-full="&nbsp;<span class='memo_more' data-op='min'>닫기</span>"
					data-min="" data-srl="2-4"></div>
				
			</div>
			<div class="clear"></div>
		</div>
		<div class="day_sch_distance">
		<img src="css/history/down.png" style="height : 20px">	
			<a
				href="javascript:et_modal('620px','700px','1','0','/ko/modal/directions?from_latlng=33.24727600,126.55482500&amp;from_name=천지연 폭포&amp;to_latlng=33.25169000,126.62246700&amp;to_name=쇠소깍','2','1');">&emsp;&emsp; &emsp;&emsp; 
				6.31km 추천경로</a>
		</div>
		<div class="day_sch_box" data-id="2-5">
			<div class="day_sch_num">
				<div class="sch_num">5</div>
			</div>
			<div class="sch_content">
				<img src="http://img.earthtory.com/img/place_img/312/7332_0_et.jpg"
					alt="" class="spot_img"
					onerror="this.src='/res/img/common/no_img/sight55.png';"
					onclick="window.open('/ko/city/jeju_312/attraction/soesokkak-estuary_7332');"
					style="cursor: pointer;">
				<div class="spot_content_box">
					<div class="spot_name"
						onclick="window.open('/ko/city/jeju_312/attraction/soesokkak-estuary_7332');"
						style="cursor: pointer;">쇠소깍</div>
					<div class="spot_info">
						<div class="tag">자연절경</div>
						<div class="sinfo_line"></div>
						<div class="sinfo_txt">
						<img src="css/history/like.png" style="height : 20px">
							8.8 / 10 <span>9개의 평가</span>
						</div>
						<div class="clear"></div>
					</div>
				</div>
				<div class="spot_btn_box">
					<img src="css/history/map_ico.png" alt=""
						class="spot_btn map_view"
						onclick="set_center(33.25169000,126.62246700)"><img
						src="css/history/info_ico.png" alt=""
						class="spot_btn spot_info_btn"
						onclick="window.open('/ko/city/jeju_312/attraction/soesokkak-estuary_7332');">
					<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
			<div class="sch_add_content">
				<div class="sch_price hide" data-srl="2-5"></div>
				<div class="sch_memo"
					data-full="&nbsp;<span class='memo_more' data-op='min'>닫기</span>"
					data-min="" data-srl="2-5"></div>
				
			</div>
			<div class="clear"></div>
		</div>
	 </div>
	<div class="clear"></div>
</div>